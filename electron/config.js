...
// --- Config Management ---
// We need to be careful with app.getPath inside a module if it's loaded before app is ready,
// but usually require happens after or during main execution. To be safe, we can defer path access or check app.isReady().
// However, typically calling app.getPath('userData') at module top-level in main process is fine in recent Electron versions *if* called after app init,
// but let's make it robust by initializing paths lazily or assuming standard invocation.
// Simpler: assume this module is required after app is initialized enough.
